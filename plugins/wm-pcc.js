...
class PCCProvider {
  ...
  async fetch(change) {
    ...
    const compileCatalogueAction = {
      ...
      callback: async () => {
        return await this.restApi.post(
          ...
          .then(() => {
            return { message: 'Puppet Catalogue Compiler triggered.' };
          })
          .catch(e => {
    ...
    if (!hasRanPuppetCompiler) {
      checkRuns.push(
        {
          checkName: 'Puppet Compiler [CI]',
          status: /** @type {RunStatus} */ ('RUNNABLE'),
          actions: [
            { ...compileCatalogueAction, name: 'Run' }
          ],
        }
      );
    }
    ...
    if (!hasRanPuppetCompiler) {
      response.actions.push(
        { ...compileCatalogueAction, name: 'Run Puppet Compiler', summary: true }
      );
    }
...
window.Gerrit.install(plugin => {
  ...
  plugin.checks().register(
    new PCCProvider(plugin.restApi()),
    {
