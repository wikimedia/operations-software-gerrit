...
QUnit.module( '[wm-checks-api]', () => {
  ...
  QUnit.module('WikimediaChecksAnalyzer', () => {
    ...
    QUnit.module('createCheckRun', () => {
      ...
      QUnit.test( 'Run links to sole build failing if any', assert => {
        const analyzer = new wmChecksApi.WikimediaChecksAnalyzer();
        analyzer.register( {
          accept: () => true,
          parse: () => [
              ...
              links: [
                {
                  primary: true,
                  icon: 'external',
                  url: 'http://ci.example.org/1',
                },
                {
                  primary: true,
                  icon: 'external',
                  url: 'http://ci.example.org/2',
                },
              ],
            },
          ],
        } );
        const checkRun = analyzer.createCheckRun(
          {},
          { tag: 'autogenerated:ci-dummy', message: '' });
        assert.propContains(checkRun, { statusLink: 'http://ci.example.org/1' } );
      } );
    ...
    QUnit.module( 'recheckActionCallback', () => {

      QUnit.test('triggers a post', async assert => {
        const analyzer = new wmChecksApi.WikimediaChecksAnalyzer(
          {
            post: async () => {},
          }
        );
        const actionResult = await analyzer.recheckActionCallback({});
        assert.deepEqual(actionResult, { message: 'Run triggered.' });
      } );

      QUnit.test('posts to a change', async assert => {
        let postedUrl;
        let postedPayload;
        const analyzer = new wmChecksApi.WikimediaChecksAnalyzer(
          {
        ...
        await analyzer.recheckActionCallback(
          {
            changeNumber: 12345,
            patchsetNumber: 99,
          },
          'test', // Name of a pipeline defined in Zuul
          'hello change!'
        );

        assert.strictEqual(postedUrl, '/changes/12345/revisions/99/review' );
      ...
      QUnit.test('handles error', async assert => {
        const analyzer = new wmChecksApi.WikimediaChecksAnalyzer(
          {
            post: async () => {
        ...
        const actionResult = await analyzer.recheckActionCallback({}, 'hello');
        assert.deepEqual(actionResult, { message: 'Triggering the run failed: POST failed' });
      } );
