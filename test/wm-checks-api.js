...
QUnit.module( '[wm-checks-api]', () => {
  ...
  QUnit.module('AWGitLabBot', hooks => {
    let awgitlabbot;
    hooks.before( () => {
      awgitlabbot = new wmChecksApi.AWGitLabBot();
    });

    QUnit.test( 'accept() from user aw-gitlab-bot', assert => {
      assert.true(
        awgitlabbot.accept( { // ChangeMessage
          real_author: { // eslint-disable-line camelcase
            username: 'aw-gitlab-bot',
          }
        })
      );
    });
    ...
    QUnit.test( 'Good job is a success', assert => {
      assert.deepEqual(
        ...
        [ {
          ...
          links: [
            {
              url: 'https://gitlab.wikimedia.org/repos/abstract-wiki/aw-e2e/-/pipelines/18789',
              primary: true,
              icon: 'external',
              tooltip: 'GitLab pipeline results'
            },
            {
              url: 'https://aw-922173.wmcloud.org/',
              primary: true,
              icon: 'external',
    ...
    QUnit.test( 'Other messages are errors', assert => {
      const result = awgitlabbot.parse( 'Patch Set 2: Verified-1\n\nEnd-to-end tests FAILED.\nResults: https://gitlab.wikimedia.org/repos/abstract-wiki/aw-e2e/-/pipelines/14825\nEnvironment: aw-904804.wmcloud.org' );
      assert.propContains(
        result[0],
        {
          category: 'ERROR',
    ...
    QUnit.test( 'Processes PAN-PAN', assert => {
      const result = awgitlabbot.parse( 'Patch Set 2:\n\nPAN-PAN: end-to-end deploy stage failed\nResults: https://gitlab.wikimedia.org/repos/abstract-wiki/aw-e2e/-/pipelines/18949' );
      assert.propContains(
        result[0],
        {
          category: 'ERROR',
          summary: 'PAN-PAN: end-to-end deploy stage failed',
        }
      );
      assert.deepEqual(
        result[0].links,
        [
          {
            url: 'https://gitlab.wikimedia.org/repos/abstract-wiki/aw-e2e/-/pipelines/18949',
            primary: true,
            icon: 'external',
            tooltip: 'GitLab pipeline results',
          },
  ...
  QUnit.module('PuppetCatalogCompiler', hooks => {
    let pcc;
    hooks.before( () => {
      pcc = new wmChecksApi.PuppetCatalogCompilerProcessor();
    });

    QUnit.test( 'accept() autogenerated:pcc-py', assert => {
      assert.true(
        pcc.accept( { // ChangeMessage
          tag: 'autogenerated:pcc-py',
        })
      );
    });
    QUnit.test( 'accept() untagged PCC messages', assert => {
      assert.true(
        pcc.accept( { // ChangeMessage
    ...
    QUnit.test( 'parse() PCC success is INFO', assert => {
      assert.deepEqual(
        pcc.parse( 'Patch Set 1: Verified+1\n\nPCC SUCCESS (NOOP 4): http://xxx'),
        [ {
          category: 'INFO',
          summary: 'Puppet compiler SUCCESS | NOOP 4',
          links: [
            {
              url: 'http://xxx',
              primary: true,
    ...
    QUnit.test( 'parse() without a node status', assert => {
      assert.propContains(
        pcc.parse( 'Patch Set 1: Verified+1\n\nPCC SUCCESS (): http://xxx')[0],
        {
          category: 'INFO',
    ...
    QUnit.test( 'parse() PCC fail is ERROR', assert => {
      const results = pcc.parse( 'Patch Set 1: Verified-1\n\nPCC FAIL (FAIL 1): http://xxx');
      assert.propContains(results[0], { category: 'ERROR' } );
    });

    QUnit.test( 'parse() PCC Check manually is WARNING', assert => {
  ...
  QUnit.module('Zuul', hooks => {
    ...
    QUnit.test( 'accept() rejects "Starting <pipeline> jobs."', assert => {
      assert.false(
        zuul.accept( { // ChangeMessage
          tag: 'autogenerated:ci-gate-and-submit',
          real_author: { // eslint-disable-line camelcase
  ...
  QUnit.module('WikimediaChecksAnalyzer', () => {
    ...
    QUnit.module( 'getCheckName', hooks => {
      ...
      QUnit.test('AWGitLabBot', assert => {
        assert.strictEqual(
          analyzer.getCheckName( {
            real_author: { // eslint-disable-line camelcase
              username: 'aw-gitlab-bot',
            },
            tag: 'autogenerated:e2e',
          } ),
          'DUCT E2E'
        );
      } );
      ...
      const pccLegacyMessages = [
        [ 'PCC Check manually: ' ],
        [ 'PCC SUCCESS (NOOP 4)' ],
        [ 'PCC FAIL (ERROR 2)' ],
      ];

      QUnit.test.each('PuppetCatalogCompiler legacy messages', pccLegacyMessages,
        ( assert, [ message ] ) => {
          assert.equal(
            analyzer.getCheckName( { message: `Patch Set 1:\n\n${message}` } ),
            'PuppetCatalogCompiler'
          );
        } );

      // Tagged since 2022-12-14
      QUnit.test('PuppetCatalogCompiler with tag', assert => {
        assert.strictEqual(
          analyzer.getCheckName( {
            tag: 'autogenerated:pcc-py',
          } ),
